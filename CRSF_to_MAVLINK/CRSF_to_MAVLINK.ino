...
#define REFRESH_INTERVAL 16  //<<<<< ----- Change to 4 for Normal CRSF/ELRS
#define CROSSFIRE_BAUD_RATE 57600 // <<<<<---- Change to 400000 for Normal CRSF/ELRS rate
...
// All parameters set here are passed through (Serial2) output in MAVLink format.
...
void setup() {
   ...
   Serial.begin(115200);// Serial Debug Output via USB/serial
   startCrossfire();  //this invokes the crossfire system on hardware Serial1
   Serial2.begin(57600);//Serial2 MAVLINK OUTPUT
}
...
void loop() {
  ...
  //Print CRSF Telem to USB/serial using variables as defined above. Add others as you wish
     ...
     Serial.print("SPEED: ");     
     Serial.println(sensorSpeed / 3.6);

     Serial.print("ALT: ");     
     Serial.println(sensorAltitude);

     Serial.print("HEADING: ");     
     Serial.println(sensorHeading);
     
     Serial.print("SATS: ");     
     Serial.println(sensorSats);

     Serial.print("LAT: ");     
     Serial.println(sensorGPSLat,7);

     Serial.print("LON: ");     
     Serial.println(sensorGPSLong,7);
  //etc..
#endif
...
void command_status(uint8_t system_id, uint8_t component_id, float sensorFuel, float sensorVoltage, float sensorCurrent) {
  ...
  // Pack the message
    mavlink_msg_sys_status_pack(system_id, component_id, &msg, 32767, 32767, 32767, 500, sensorVoltage * 1000.0, sensorCurrent * 100.0, sensorFuel, 0, 0, 0, 0, 0, 0);
